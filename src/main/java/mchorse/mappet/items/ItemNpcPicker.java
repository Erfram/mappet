...
public class ItemNpcPicker extends Item {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        tooltip.add(IKey.lang("item.mappet.npc_picker.tooltip").toString());
    }
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase target, EnumHand hand) {
        ...
        if (id.isEmpty()) {
            player.sendStatusMessage(new TextComponentString(IKey.lang("item.mappet.npc_picker.message.notid").toString()), true);
            return false;
        }

        this.saveNPC(npcState);

        player.sendStatusMessage(new TextComponentString(IKey.format("item.mappet.npc_picker.message.savednpc",  id).toString()), true);
    ...
    private boolean replaceItem(EntityPlayer player, ItemStack stack, EnumHand hand) {
        ItemStack npcSoulstoneEmpty = new ItemStack(MappetItems.npcSoulStoneEmpty);
